...
class Entreprise:
    ...
    def payer_salaires(self):
        ...
        if self.argent - total >= self.decouvert_max:
            self.argent -= total
            print(f"💸 Paiement des salaires : -{total}€ ({self.ouvriers} ouvriers, {self.ingenieurs} ingénieurs)")
        else:
            print(f"❌ Paiement des salaires annulé : cela dépasserait le découvert autorisé ({self.decouvert_max}€).")
    ...
    def verifier_sauvetage_ou_fail(self):
        ...
        elif self.argent < self.decouvert_max:
            print("\n💥 Vous avez dépassé le découvert autorisé sans soutien restant des actionnaires.")
